...
from ..const import (FILENAME_SUBSTITUTIONS_DELETE_WORDS, FILENAME_SUBSTITUTIONS_FULL_WORDS,
                     FILENAME_SUBSTITUTIONS_PARTIAL_WORDS)


class Preprocessing:
    
    @staticmethod
    def replace_corrections_vl(column: pd.Series) -> pd.Series:
    ...
    def substitute_partial_words(column: pd.Series, data_root: str) -> pd.Series: 
        ...
        sub = pd.read_csv(Path(data_root) / FILENAME_SUBSTITUTIONS_PARTIAL_WORDS, sep = ";", header = None, names = ["abbreviation", "expansion"], comment='#', encoding ='utf-8')
        ...
        # do replacement 
        return column.str.lower().replace(to_replace=subst_dict, regex=True)
    
    @staticmethod      
    def substitute_delete_words(column: pd.Series, data_root: str) -> pd.Series: 
        ...
        # load defined abbreviations 
        sub = pd.read_csv(Path(data_root) / FILENAME_SUBSTITUTIONS_DELETE_WORDS, sep = ";", header = None, names = ["abbreviation", "expansion"], comment='#', encoding ='utf-8') 
    ...
    @staticmethod      
    def substitute_full_words(column: pd.Series, data_root: str) -> pd.Series: 
        """Function no 3. for substituting abbreviations: substitutes specific abbreviations"""
        
        # load defined abbreviations 
        sub = pd.read_csv(Path(data_root) / FILENAME_SUBSTITUTIONS_FULL_WORDS, sep = ";", header = None, names = ["abbreviation", "expansion"], comment='#', encoding ='utf-8')
